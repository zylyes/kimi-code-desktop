...
- 自绘窗控（2026-08）：min/max/close 三钮挂 `.app-topbar-actions` 末尾（☰ 右侧），**禁用同名 `.kcd-menu-btn`**——menu-panel.js 的 mount 判定（`querySelector('.kcd-menu-btn')`）与自愈重挂会把它误认成 ☰ 导致菜单丢失；新类 `.kcd-win-btn` 逐字复刻样式（38×32、透明、radius:0、hover #00000012 / 暗色 #ffffff1a 两条命中）。脚本时序：普通脚本先于 defer 执行，故 usage.js 必须 defer 且列于 menu-panel.js 之后，按钮 append 才位于 ☰ 右侧。页面级覆盖 `.app-topbar { padding-right: 10px }` 收回 OS overlay 的 150px 让位（依赖主进程去 overlay）。静态 SVG 图标常量 innerHTML 写入与 menu-panel.js:211 同款，无动态拼接。windowControl 调用防御：typeof 判函数 + try/catch + promise .catch 吞 rejection。
