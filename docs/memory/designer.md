...
## workspace.html / workspace.css / workspace.js 页面约定（2026-08，M2）

- 工作区面板：主窗口右侧覆盖式 WebContentsView（固定 360px，主进程 `layoutWorkspaceView` 顶部偏移 36px），页面从 y=0 布局，**不带** drag-strip / app-topbar / menu-panel.js / 窗控；左缘 1px `--separator` 与 Web UI 划界。
- 桥 `window.workspace`（preload workspace-preload.js）：getContext / getPanelState / setPanelState / selectCandidate / onEvent（kind==='context' 刷新头部）；全 try/catch，失败进内联 `.err-bar`。
- 头部三态：bound（截断 sid 头12…尾4 + confidence badge + workDir 中间省略 + 「更新于 HH:MM:SS」本地手动 pad，不用 toLocaleTimeString）/ candidates（列表 max-height 132px 可滚，hover/active 用 --bg-secondary-*）/ unbound（共享 .spinner + 次要文字）。
- **破例点**：`--color-warning` 警示黄是 workspace.css 页面级令牌（主题仅 error/success 两彩色），按主题同款暗色双命中（媒体查询 + .kcd-page-dark）定义，待主题层收编；浅色 #9c7600 / 暗色 #ffd60a。
- 标签栏：role=tablist + roving tabindex + 方向键/Home/End；激活指示 = 2px 下划线（border-bottom 常占 2px 透明防位移），过渡 0.15s；空态图标用 Unicode（± ▤ ✦ ☑，46px 圆形容器 bg-secondary）。
- M3 Changes/Files（2026-08）：新增桥 getChanges/listFiles/readFile/getDiff + onEvent kind:'refresh'。结构 `.pane-note`（说明条）+ `.pane-body`（flex 列、内滚）撑满 `.tabpane`；空态/失败/加载由 JS 渲染进 pane-body（renderPaneEmpty 复用 M2 .empty，renderPaneFail 带「重试」btn-secondary）。
- Changes：status 单字母徽标 A/M/D/R/U（绿/黄/红/蓝/灰，color-mix 徽章手法；蓝色 = 新增页面级 `--color-info` #007aff/#0a84ff，与 --color-warning 同双命中模式）；行 = head 按钮（line1 徽标+path+chevron ▸ 旋转，line2 「暂存」「未暂存」两 stat-block 胶囊，双零 .zero 降不透明度）；点击展开 diff（snapshotId 快照守卫丢弃在途、快照换即清 diffs 缓存、pruneDiffs 按 entry id 清理），diff 行级着色 dl-add/dl-del/dl-hunk（排除 +++/--- 头），渲染上限 2000 行防卡。
- Files：目录懒加载（children Map + expanded Set，全量刷新保留展开并重拉所有展开目录）；树行单行 30px（name 弹性省略 + 右侧 mono meta `size · YYYY-MM-DD HH:mm`）；复制路径钮 absolute 右置、行 hover 浮现盖住 meta 防位移、「已复制」1.2s 绿反馈；预览抽屉 `.preview` 树下方（max-height 42%、bg-primary、Escape/×关闭、binary-file/截断脚注专态）；选中行 .active 高亮。
- 刷新纪律：tab 首屏切到才加载（loadedTabs），切回用缓存 + background 静默刷新（失败保留旧数据 + err-bar）；onEvent refresh/context → 3s 防抖（scheduleRefresh，连续合并）只刷当前激活标签；listFiles 经 callListFiles 归一化（catch → {ok:false,reason}）使 Promise.all 不中断。自检：Temp/opencode/m3-workspace-selfcheck.cjs 桩 DOM 模式 28 断言。
- 跨会话防泄漏（2026-08-04，Oracle 高危整改 + 最终门禁）：`contextKey`（bound=sessionId+workDir，未绑定='unbound:<state>' 稳定占位，getContext 失败='unbound:context-error' 稳定失败键，null=初始未建立）+ `contextGeneration` + `contextRequestSeq` 双单调递增。事件约定：main 仅在导航 fingerprint/候选选择致身份可能变化时发 `{kind:'context'}`（按潜在身份切换处理），同身份 focus 发 `{kind:'refresh'}`（仅 3s 防抖，绝不清空数据）。
- M4 Agents/Tasks 只读投影（2026-08-04）：桥 `getProjection()`（{ok,sessionId,agents:SubagentNode[],tasks:CatalogEntry[],capturedAt}）+ onEvent kind:'activities'（payload.sessionId 匹配当前 bound 才 400ms 短防抖重拉，缺/异 sessionId 不动）。两标签共享单一 `projection` state（loaded/status/agents/tasks/agentExpanded/seq），invalidateForContextRefresh 同步清投影+activitiesTimer+note 复位，establishContext 对 agents/tasks 同样立即重载。Agents：合成 main 根行（计数摘要）+ parentAgentId 嵌套（.ag-children）+ 父缺失/成环节点归「未知关系（N）」虚线组；状态 8px 色点（running=info/completed=success/failed=error/interrupted=warning/unknown=灰）；步骤可展开（无步骤 head disabled），步骤文本 96 字截断、单代理 50 步上限、绝不渲染工具 output；note 显示「会话快照 · HH:MM:SS」（文案不宣称实时流）。Tasks：updatedAt 倒序单列表，kind 徽标（task=info/subagent=success/cron=warning color-mix 徽章手法）+ 来源 badge（ws 实时同步=success/acp 会话事件=info/disk 本地文件=中性）+ 置信度（high 不显示/medium 参考/low 粗略=warning，与 chat 页任务抽屉同语言）+ cron detail 短字段（schedule/cron/expression/interval，28 字截断）+ mono 更新时间；terminal/failed/removed 行 .dim 0.55；零 button 纯只读。空态/未绑定/错误全走 renderPaneEmpty/renderPaneFail 既有语言。自检 Temp/opencode/m4-workspace-selfcheck.cjs 50 断言。
- 最终门禁两阶段（2026-08-04）：refreshContext 开始即 `invalidateForContextRefresh()`（gen++ + contextReady=false + 同步清空 Changes/Files/diff/preview/loadedTabs/旧 DOM + 取消待触发防抖；getContext 落地前不发新数据请求，此间切标签只切视觉）；落地且过 seq 核对才 `establishContext()`（成功=新非 null key + renderContext + 当前标签立即重载不走防抖；失败=失败键定型同样放行，先 establish 后 showError 防 clearError 抹条）。staleContext：key===null 一律判 stale + gen/key 严格双等，无 null 放行通道。自检同文件 111 断言（新增：严格时序 12「A 数据在途→B context 已发未落地→A 返回绝不写回」+ 失败定型 4 + null-key 全新页面场景 9；既有 84 条中 2 条按新契约改写为「同身份 context=立即失效清空+落地立即重载」并扩至 4 条，其余 82 条原样通过）。
- M6 overlay 关闭安全恢复（2026-08-04，修"旧 DOM 一帧暴露"）：overlay 关闭且 context stale 时，保有旧 DOM 的 view **绝不直接挂回**——主进程 `workspace-restore.js` 状态机（纯逻辑、deps 注入、node 可测）在隐藏态发 `{kind:'context', restoreId}`，页面走常规 refreshContext（invalidate 同步清 DOM → getContext 落地 → establish）后经 preload `ackContextRestore(restoreId)` → `workspace:contextRestored`（guard `validateContextRestored`：正整数 ≤2^31-1）回执，ack 匹配（id+view 引用）才 `mountWorkspaceViewOnce` 单次挂回；ack 超时 3s → 受控 reload workspace.html 再发同 id 等 5s，load 失败 fail-closed 不挂回，reload 后仍超时兜底挂回（旧 DOM 已销毁）。取消挂点：showOverlay（再次 overlay）/hideWorkspacePanel（折叠销毁）/mainWindow closed；恢复 pending 期 pushWorkspaceEvent 全拦截、showWorkspacePanel 直接挂载被 isPending 把守（loadMain 场景）。无 stale → mountWorkspaceViewOnce 零重载快速恢复；焦点恒在主 Web。测试 `tests/test-workspace-restore.js` 11 例 + Temp `m6-workspace-restore-selfcheck.cjs` 18 断言（ack 时机/同步清 DOM/在途不写回/失败也回执/ack 抛错不致命）。教训：loadChanges 成功分支 clearError() 会抹掉 context 失败错误条——自检失败定型场景须让数据桥也失败。
